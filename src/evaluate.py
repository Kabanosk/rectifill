...
@torch.no_grad()
def evaluate(checkpoint_path: str, data_path: str, output_dir: str, device: str, batch_size: int, min_mask: int, max_mask: int):
    output_dir_path = Path(output_dir)
    samples_dir = output_dir_path / "best_samples"
    samples_dir.mkdir(parents=True, exist_ok=True)

    train_config = TrainConfig()
    data_config = DataConfig(batch_size=batch_size, shuffle=False, data_path=data_path, drop_last=False)
    ...
    for batch in tqdm(test_loader, desc="Evaluation"):
        ...
        gen_mel_norm = sample_euler(
            model=model, x1_context=mel_norm, mask_bool=mask_bool, num_steps=50, cfg_scale=train_config.cfg_scale,
            verbose=False, **condition_kwargs
        )
        gen_mel_db = denormalize_mel(gen_mel_norm)
        t_batch = (time.perf_counter() - t0) / mel_raw.shape[0]

        for b in range(mel_raw.shape[0]):
            valid_len = (~mel_pad_mask[b]).sum().item()

            mel_raw_b = mel_raw[b, :, :valid_len]
            gen_mel_db_b = gen_mel_db[b, :, :valid_len]
            mask_bool_b = mask_bool[b, :, :valid_len]
            ...
            try:
                ...
                wer = asr_evaluator.calculate_wer(batch['text'][b], pred_text)
            except Exception as e:
            ...
            stats["gen_time"].append(t_batch)
            stats["audio_len"].append(duration)
            stats["rtf"].append(t_batch / duration)
...
if __name__ == "__main__":
    ...
    evaluate(args.model, args.data_path, args.output_dir, device, args.batch_size, args.min_mask, args.max_mask)

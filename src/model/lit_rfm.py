...
class LitRFM(pl.LightningModule):
    ...
    def training_step(self, batch: dict, batch_idx: int) -> torch.Tensor:
        ...
        mel_pad_mask = batch['mel_padding_mask']
        text_emb = batch['embedding']
        text_mask = batch['text_padding_mask']
        ...
        v_pred = self.model(
            xt=xt, mask=mask_float, t=t,
            text_emb=text_emb, text_mask=text_mask, mel_pad_mask=mel_pad_mask
        )

        loss = F.mse_loss(v_pred, target_v, reduction='none')
        masked_loss = loss[mask_bool.expand_as(loss)].mean()

        self.log("train/loss", masked_loss, prog_bar=True, on_step=True, on_epoch=True)
        return masked_loss
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> None:
        ...
        loss = F.mse_loss(v_pred, target_v, reduction='none')
        masked_loss = loss[mask_bool.expand_as(loss)].mean()
        self.log("val/loss", masked_loss, prog_bar=True, sync_dist=True)

        if batch_idx < self.config.validation_metrics_steps:
            generated_mel_norm = sample_euler(
                model=eval_model, x1_context=mel, mask_bool=mask_bool,
                text_emb=text_emb, text_mask=text_mask, mel_pad_mask=mel_pad_mask,
                num_steps=50, cfg_scale=self.config.cfg_scale
            )
            generated_mel = denormalize_mel(generated_mel_norm)
            batch_lsd = calculate_lsd(generated_mel, mel_raw, mask_bool)
            self.log("val/lsd", batch_lsd, sync_dist=True, prog_bar=True)

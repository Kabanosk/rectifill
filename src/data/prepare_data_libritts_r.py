...
def load_transcripts_libritts(source_dir: Path) -> dict[str, str]:
    """
    LibriTTS/-R holds transcripts differently than LibriSpeech.
    Each audio file has a corresponding .normalized.txt file instead of a collective .trans.txt
    """
    transcripts = {}
    logger.info(f"Searching for LibriTTS transcripts in: {source_dir}...")

    for txt_file in source_dir.rglob("*.normalized.txt"):
        file_id = txt_file.name.replace(".normalized.txt", "")
...
def process_audio_and_text_libritts(source_dir: Path, output_dir: Path, config: DataConfig):
    """
    Processes audio files (resampling), generates log-mel spectrograms and T5 embeddings/phonemes,
    and saves .pt + .txt pairs, driven by the provided configuration.
    Adapted for LibriTTS (.wav files, different transcript structure).
    """
    ...
    transcripts = load_transcripts_libritts(source_dir)
    ...
    logger.info(f"Found {len(wav_files)} .wav files. Processing...")

    metadata = []

    for wav_path in tqdm(wav_files, desc="Processing LibriTTS-R"):
...
def download_and_prepare_libritts(config: DataConfig, dataset_type: str = "train-clean-360", output_subdir: str = ""):
    """
    Downloads and extracts the LibriTTS-R dataset from OpenSLR, then triggers the processing script.
    """
    ...
    if not output_subdir:
        output_subdir = dataset_type
    processed_path = root_path / "processed" / output_subdir

    logger.info(f"--- Starting work on LibriTTS-R dataset: {dataset_type} ---")

    file_name = f"{dataset_type.replace('-', '_')}.tar.gz"
    download_url = f"https://www.openslr.org/resources/141/{file_name}"
    archive_path = raw_path / file_name

    try:
        if not archive_path.exists():
            logger.info(f"Downloading LibriTTS-R from {download_url} (This may take a while)...")
            urllib.request.urlretrieve(download_url, archive_path)
            logger.info("Download complete.")

        extracted_base = raw_path / "LibriTTS_R"
        extracted_path = extracted_base / dataset_type

        if not extracted_path.exists():
            logger.info(f"Extracting {archive_path} to {raw_path}...")
    ...
    process_audio_and_text_libritts(extracted_path, processed_path, config)
...
if __name__ == "__main__":
    ...
    download_and_prepare_libritts(main_config, dataset_type="dev-clean")

    logger.info("Starting processing: COMBINED TRAINING SET (100h + 360h -> train-clean)")
    download_and_prepare_libritts(main_config, dataset_type="train-clean-100", output_subdir="train-clean")
    download_and_prepare_libritts(main_config, dataset_type="train-clean-360", output_subdir="train-clean")

    logger.info("Starting processing: TEST SET")
    download_and_prepare_libritts(main_config, dataset_type="test-clean")
